#!/usr/bin/env python3
"""Controle pre-vol : a lancer AVANT d'installer le plugin, sur la machine cible.

    python3 scripts/preflight.py     (ou : python scripts\\preflight.py)

Verifie l'environnement et l'integrite du plugin. N'installe rien.
"""
import argparse
import json
import os
...
ap = argparse.ArgumentParser(description="Controle pre-vol du plugin.")
ap.add_argument("--fix-hooks", action="store_true",
                help="reecrit hooks/hooks.json avec l'invocation Python qui fonctionne ici")
ARGS = ap.parse_args()

print(f"Plugin : {RACINE}\n")
print("Environnement")
test(f"Python {sys.version_info.major}.{sys.version_info.minor} (3.9+ requis)",
     sys.version_info >= (3, 9), f"version trouvee : {sys.version.split()[0]}")
print(f"         invocation a utiliser dans les commandes : {os.path.basename(sys.executable)}")
try:
...
def invocation_qui_marche() -> str | None:
    ...
    qui echouent, alors que le launcher `py` fonctionne. L'inverse est vrai sous
...
test("une invocation Python fonctionne depuis un shell", INVOCATION is not None,
     "ni py, ni python3, ni python ne repondent")
if INVOCATION:
...
if os.path.isfile(manifeste):
    try:
        ...
        test("champ 'name' present", bool(decl.get("name")))
    except json.JSONDecodeError as exc:
        test("plugin.json est un JSON valide", False, str(exc))

for chemin in decl.get("agents", []):
    p = os.path.join(RACINE, chemin.lstrip("./"))
    test(f"agent declare present : {os.path.basename(chemin)}", os.path.isfile(p))

hooks = os.path.join(RACINE, "hooks", "hooks.json")
test("hooks/hooks.json present", os.path.isfile(hooks))
if os.path.isfile(hooks):
    try:
        ...
        # L'invocation ecrite dans hooks.json doit etre celle qui marche ICI.
        # Sinon les portes qualite ne s'executent jamais — en silence.
        actuelle = None
        for cand in ("py -3", "python3", "python"):
        ...
        if not coherent and ARGS.fix_hooks and INVOCATION and actuelle:
            ...
            print(f"         -> hooks.json corrige : '{actuelle}' remplace par '{INVOCATION}'")
            KO.pop()
            OK.append(("hooks.json corrige", ""))
    except json.JSONDecodeError as exc:
        test("hooks.json est un JSON valide", False, str(exc))

print("\nIntegrite des agents")
...
for nom in sorted(os.listdir(dossier)) if os.path.isdir(dossier) else []:
    ...
    test(f"{base} : aucune reference @fichier residuelle", "@_COMMUN.md" not in t)
    test(f"{base} : aucun ${{CLAUDE_PLUGIN_ROOT}} (non substitue dans un agent)",
...
try:
    ...
    test(f"{len(regles)} regles chargees (11 attendues)", len(regles) == 11)
except Exception as exc:
    test("chargement des regles", False, str(exc))
...
if os.path.isdir(exemple):
    ...
    try:
        ...
        test("l'exemple de reference detecte bien ses defauts volontaires",
             d["bloquants"] > 0, f"{d['bloquants']} ecart(s) attendus")
    except Exception:
        test("execution du validateur sur l'exemple", False, r.stderr[-200:])

print(f"\n{len(OK)} controle(s) OK, {len(KO)} en echec.")
if KO:
    print("\nA regler avant installation :")
    for nom, detail in KO:
        print(f"  - {nom}" + (f" ({detail})" if detail else ""))
    sys.exit(1)
print("Pret pour l'installation. Voir INSTALLATION.md.")
if INVOCATION:
